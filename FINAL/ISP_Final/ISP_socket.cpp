			...
			while(ISPStart) {
				...
				if(faces.size() != 0)
				{
					...
					for (unsigned long i = 0; i < faces.size(); ++i) {
						...
						//shapes[i] = pose_model(cimg, faces[i]);
						shapes.push_back(shape);

						//cout << " debug3......................... " << endl; 
						//distance[i] = abs(340 - (int)shapes[i].part(33).x()) + abs(235-(int)shapes[i].part(33).y());
						//distance[i] = abs(340 - (int)shape.part(33).x()) + abs(235-(int)shape.part(33).y());
						distance.push_back(pow((340 - (int)shape.part(33).x()), 2) + pow((235-(int)shape.part(33).y()), 2));
					...
					std::cout << "x_cor : " << (int)shape.part(33).x() << std::endl;
					std::cout << "y_cor : " << (int)shape.part(33).y() << std::endl;
					...
					//Up, Down, Left, Right
					if(y_face < 200) strcat(s_msg,"[3]U");
					else if(y_face > 280) strcat(s_msg,"[3]D");
					else strcat(s_msg,"[3]N");

					if(x_face < 280) strcat(s_msg,"@R");
					else if(x_face > 360) strcat(s_msg,"@L");
					else strcat(s_msg,"@N");

					//Far, Close
					if(dist_to_center > 0.550) strcat(s_msg, "@F\n");
					else if(dist_to_center < 0.350) strcat(s_msg, "@C\n");
					else strcat(s_msg,"@N\n");
					...
					no_face_cnt++;
					cout <<"Noface count : " << no_face_cnt << endl;
					if((no_face_cnt > 30) && (strcmp(s_msg, "[3]NOFACE\n\0") != 0)){
						cout <<"NOFACE write!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
						memset(s_msg,0,sizeof(s_msg));
						strcpy(s_msg,"[3]NOFACE\n\0");
						write(sock, s_msg, strlen(s_msg));
					}
				}
				//cv::resize(temp, temp_full, Size(1920, 1080));
				//cv::imshow("OpenCV Landmark Detector", temp_full);
